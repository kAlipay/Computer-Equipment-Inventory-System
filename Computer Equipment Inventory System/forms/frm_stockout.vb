Public Class frm_stockout
    Dim returnid As Integer
    Dim stockinID As Integer = 0
    Dim stockoutID As Integer = 0
    Dim stockreturnID As Integer = 0
    Private Sub frm_stockout_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub txtcustomerid_TextChanged(sender As Object, e As EventArgs) Handles txtcustomerid.TextChanged
        Try
            sql = "SELECT * FROM `tblperson` WHERE `CustomerID`='" & txtcustomerid.Text & "'"
            reloadtxt(sql)
    ...
    Private Sub btnsave_Click(sender As Object, e As EventArgs) Handles btnsave.Click
        ...
        For Each row As DataRow In dt.Rows
            For i As Integer = 0 To dtg_addedlist.Rows.Count - 1
                If dtg_addedlist.Rows(i).Cells(0).Value = row.Item(0) Then
                    'MsgBox(row.Item(0))
                    If dtg_addedlist.Rows(i).Cells(4).Value > row.Item(1) Then
                        MsgBox("The Quantity of the item ( " & dtg_addedlist.Rows(i).Cells(1).Value & " ) is greater than the available quantity of it.", MsgBoxStyle.Exclamation)
                        Exit Sub
                    End If

                End If
                If dtg_addedlist.Rows(i).Cells(4).Value = "" Then
        ...
        For Each r As DataGridViewRow In dtg_addedlist.Rows
            ...
            "','" & r.Cells(5).Value & "','" & txtcustomerid.Text & "','StockOut')"
            createNoMsg(sql)
            '-----------------------------------------------update item
       ...
       " VALUES ('" & stockoutID & "','" & Format(Now, "yyyy-MM-dd") & "','StockOut','" & txtcustomerid.Text & "')"
        createNoMsg(sql)
        '-----------------------------------------------update autonumber
    ...
    Private Sub btnviewlist_Click(sender As Object, e As EventArgs) Handles btnviewlist.Click
        frm_viewstockout.Show()

    End Sub

    Private Sub dtg_addedlist_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles dtg_addedlist.CellBeginEdit
        Try
            Dim total As Double
    ...
    Private Sub dtg_addedlist_CellMouseLeave(sender As Object, e As DataGridViewCellEventArgs) Handles dtg_addedlist.CellMouseLeave
        Try
            Dim total As Double
            For Each row As DataGridViewRow In dtg_addedlist.Rows
                total = row.Cells(4).Value * row.Cells(3).Value
    ...
    Private Sub dtg_addedlist_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dtg_addedlist.CellValueChanged
        Try
            Dim total As Double
            For Each row As DataGridViewRow In dtg_addedlist.Rows
                total = row.Cells(4).Value * row.Cells(3).Value
